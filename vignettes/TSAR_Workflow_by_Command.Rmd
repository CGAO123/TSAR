...
gammaresult <- model_fit(test, model = gammodel)
```
...
```{r}
Tm_est(gammaresult)
view <- view_model(gammaresult)
view[[1]] + theme(aspect.ratio = 0.7, legend.position = "bottom")
view[[2]] + theme(aspect.ratio = 0.7, legend.position = "bottom")
```
Alternatively, user can also use `model_beta()` to fit a beta model to the data.

```{r}
betamodel <- model_beta(test, x ="Temperature", y = "Normalized")
view <- view_model(betamodel)
view[[1]] + theme(aspect.ratio = 0.7, legend.position = "bottom")
view[[2]] + theme(aspect.ratio = 0.7, legend.position = "bottom")
```
...
What's new:
User can now choose the smoother type ("gam", "beta", or "none") to be used in the plot.
...
TSA_wells_plot(error, y='Fluorescence', separate_legend = TRUE, smoother = "beta", beta_shape = 4, beta_knots_frac = 0.008, show_Tm = TRUE)
```
