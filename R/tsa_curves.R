...
TSA_wells_plot <- function(
    tsa_data,
    y = "RFU", #c("Fluorescence", "RFU")
    show_Tm = TRUE, #c(T, F) Logical, to show Tm
    Tm_label_nudge = 7.5, #If show_Tm = T, moves label Right (+) or Left (-). 7.5 by default
    show_average = TRUE,
    plot_title = NA, #Specify Plot title
    plot_subtitle = NA, #Specify Plot subtitle
    smooth = TRUE,
    #Speeds up plotting when true
    separate_legend = TRUE #Return legend as separate plot.
) {
    ...
    if(is.na(plot_title)){ #Auto-generate plot titles if needed
        if(TSA_proteins(tsa_data = tsa_data, n = T) == 1){
            title <- paste0(c("Thermal Profile of ",
                              TSA_proteins(tsa_data = tsa_data)),
                            collapse = "")
        } else { title <- "Thermal Profile" }
...
TSA_boxplot <- function(
    ...
    plot_data <- TSA_Tms(analysis_data = tsa_data,
    ...
    if(!is.na(control_condition)) {
        if(control_condition %in% condition_IDs(tsa_data)) {
            ...
            stop("condition_ID assigned to control_condition is not found in the TSA data.
               Use condition_IDs(tsa_data) to get the list of acceptable IDs")
        }
    }
...
tsa_compare_plot <- function(
    tsa_data,
    control_condition, #The condition_ID used to compare
    y = "Fluorescence", #c("Fluorescence", "RFU")
    show_Tm = FALSE,
    ...
    for(i in 1:condition_IDs(tsa_data, n = TRUE)) {
        ...
        Tm_diff_i <-
            Tm_difference_DF$delta_Tm[Tm_difference_DF$condition_ID == condition_i]
