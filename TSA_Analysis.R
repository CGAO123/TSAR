...
#' approximated by locating maximum derivative
#'
#' @param data data frame; data frame input containing derivative values
...
#' @importFrom magrittr %>%
#'
#' @param rawdata data frame; raw data frame
...
#' tm estimation bby well, dataset, fit of model by well
#'
#' examples
...
gam_analysis <- function(rawdata,
    ...
    for(i in unique(rawdata$Well.Position)){
        by_well <- rawdata %>%
            filter(Well.Position == i)
        #normalize data
        by_well <- normalize(data = by_well, Fluo = 5, selected = selections)

        #check is data is already smooth
        #fit model is not smoothed
        if(smoothed == F){
            gammodel <- model_gam(Data = by_well,
            ...
            by_well <- modelfit(data = by_well, model = gammodel)
        }else{
    ...
    #typify all return values into data frames and cat into list of data frames
    tm <- data.frame(tm)
    kept <- data.frame(kept)
    tobereturned <- list(tm)

    #concat by users' specifications
    if(keep == T){ tobereturned <- append(tobereturned, list(kept)) }
    if(fit == T){ tobereturned <- append(tobereturned, list(fitsummaries)) }
    #reurn list of data frames
    return(tobereturned)
