...
Users may access TSAR workflow from either `raw_data` or `norm_data` as long as data achieves proper qualities. Functions corresponding each tier are wrapped in shiny application. All analysis and visualization can be achieved within user-interactive window, also open-able in local browser.

- `weed_raw()`; input: raw; output: data without blank wells and corrupted curves
- `analyze_norm()`; input: raw or output of weed_raw; output: analyzed data with conditions specified
...
```{r}
raw_data <- remove_raw(raw_data, removelist = 
                           c("B04", "B11", "B09", "B05", "B10", "B03", 
...
```{r message=FALSE}
output <- join_well_info(file_path = NULL, file = Well_Information, 
                         read_tsar(x, code = 0), type = "by_template")
```
...
All of the function above in chapter 2 are wrapped together in a shiny application named `analyze_norm()`. Refer to separate vignette, `"TSAR Workflow by Shiny"`, and readme.md file for more documentation.
...
norm_data_rep <- join_well_info(file_path = NULL, file = Well_Information, 
                                read_tsar(analysis_rep, code = 2), 
                                type = "by_template")
...
All of the above functions are also wrapped in an interactive window call through `graph_tsar()`. Simply call function on merged tsar_data and access all graphing features in one window. Refer to separate vignette, `"TSAR Workflow by Shiny"`, and readme.md file for more documentation.
