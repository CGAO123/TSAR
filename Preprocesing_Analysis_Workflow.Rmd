...
```{r}
test <- raw_data %>%
  filter(Well.Position == "A03") # select data for one well by well ID
...
test <- model_fit(data = test, model = gammodel)
...
write_tsar(read_tsar(x, code = 2), name = "tm_val", file = "csv")
...
norm_data <- join_well_info("/Users/candygao/Desktop/qpcrresult/Well Information Template.xlsx", output, type = "by_template")
head(norm_data)
```
<<<<<<< HEAD
=======
